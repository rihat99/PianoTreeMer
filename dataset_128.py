...
def calculate_low_high(dataset_path):
    ...
    for data in dataset:
        ...
        sustain_data = data[:, :] == 1
        silence_data = data[:, :] == 0
        pr = np.stack([onset_data, sustain_data, silence_data], axis=2).astype(bool)
        # pr: (32, 128, 3)
...
def reshape_grid_to_plot(piano_grid):
    ...
    y = piano_grid.transpose([1, 0, 2]).reshape((-1, 8*128))
    return y


if __name__ == '__main__':
    file_path = r'/home/vladislav/Documents/Studies/ML711/Final project/pop909+mlpv_t32_fix1'
    # l, h, num_notes = calculate_low_high(file_path + '/pop909+mlpv_t32_val_fix1.npy')
    # print(l, h, num_notes)
    train_dataset = PolyphonicDataset(file_path + '/pop909+mlpv_t32_train_fix1.npy', -3, +3)  # DO augment on training dataset!

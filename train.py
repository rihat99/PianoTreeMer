...
#from model_transf import VAE
#from PianoTree import VAE_modified as VAE
...
# Load config
###############################################################################
config_fn = './model_config_orig.json'
...
MIN_LR = train_hyperparams['min_lr']
###############################################################################
# Initialize project
###############################################################################
...
runs_path = f'runs_my_decoder'
run_id = time.strftime("%Y-%m-%d_%H-%M-%S")
run_name = f'run_{run_id}_{BATCH_SIZE}_{LEARNING_RATE}_{N_EPOCH}'
runs_path = os.path.join(runs_path, run_name)
MODEL_PATH = os.path.join(runs_path, MODEL_PATH)
LOG_PATH = os.path.join(runs_path, LOG_PATH)
...
# load data
###############################################################################
# train_path = os.path.join(dataset_path, 'pop909+mlpv_t128_train.npy')
...
if INIT_WEIGHT:
    ...
    # load model weights
    model.load_state_dict(torch.load('./runs/run_2024-04-05_20-16-11_512_0.001_6/models_enc_tr/pgrid-last-model.pt'))
...
if DECAY:
    # scheduler = MinExponentialLR(optimizer, gamma=0.9999, minimum=MIN_LR)
    scheduler = CosineAnnealingWarmupRestarts(optimizer, 
                                              first_cycle_steps=8100, 
                                              cycle_mult=1.0, 
...
def train(model, 
    ...
    for i, batch in tqdm(enumerate(train_loader)):
        ...
        loss, pitch_loss, dur_loss, kl_loss = loss_function(recon_pitch,
        ...
        loss_writer.add_scalar('batch_train_loss %d' % num_batch, loss.item(), epoch * num_batch + i)
        kl_writer.add_scalar('batch_train_loss %d' % num_batch, kl_loss.item(), epoch * num_batch + i)
        pitch_writer.add_scalar('batch_train_loss %d' % num_batch, pitch_loss.item(), epoch * num_batch + i)
        dur_writer.add_scalar('batch_train_loss %d' % num_batch, dur_loss.item(), epoch * num_batch + i)

        # print(f'Batch: {i + 1:02} | Dur: {dur_loss.item()}, Pitch: {pitch_loss.item()}', flush=True)

        if decay:
            scheduler.step()

    return (epoch_loss / num_batch, 
            epoch_pitch_loss / num_batch,
            epoch_dur_loss / num_batch, 
...
def evaluate(model, 
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(val_loader)):
            ...
            recon_pitch, recon_dur, dist = model(batch, 
                                                 False, 
                                                 True,
                                                 tfr1, 
                                                 tfr2)
            ...
            epoch_dur_loss += dur_loss.item()
            epoch_kl_loss += kl_loss.item()
            loss_writer.add_scalar('batch_eval_loss %d' % num_batch, loss.item(), epoch * num_batch + i)
            kl_writer.add_scalar('batch_eval_loss %d' % num_batch, kl_loss.item(), epoch * num_batch + i)
            pitch_writer.add_scalar('batch_eval_loss %d' % num_batch, pitch_loss.item(), epoch * num_batch + i)
            dur_writer.add_scalar('batch_eval_loss %d' % num_batch, dur_loss.item(), epoch * num_batch + i)
...
for epoch in range(N_EPOCH):
    ...
    train_loss, train_pl, train_dl, train_kl = train(model, 
                                                    train_loader,
                                                    optimizer, 
                                                    pitch_loss_func, 
                                                    dur_loss_func, 
    ...
    valid_loss, valid_pl, valid_dl, valid_kl = evaluate(model, 
                                                        val_loader,
                                                        pitch_loss_func, 
                                                        dur_loss_func, 
                                                        normal,
    ...
    torch.save(model.state_dict(), os.path.join(MODEL_PATH, 'pgrid-epoch-model.pt'))

    if valid_loss < best_valid_loss:
        best_valid_loss = valid_loss
        torch.save(model.state_dict(), os.path.join(MODEL_PATH, 'pgrid-best-valid-model.pt'))

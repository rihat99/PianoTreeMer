...
class VAETR(nn.Module):
    def __init__(self, 
        ...
        self.linear_enc_time = nn.Linear(self.enc_time_hid_size, 8)
        ...
        # decoder
        self.dec_time_token = nn.Parameter(torch.randn(128, 512))
    ...
    def decoder(self, z):
        ...
        mask = (torch.triu(torch.ones(128, 128)) == 1).transpose(0, 1)
    ...
    def index_tensor_to_multihot_tensor(self, ind_x):
        ...
        with torch.no_grad():
            ...
            out = out.view(-1, 128, self.max_simu_note, self.pitch_range + 1)

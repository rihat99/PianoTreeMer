...
class VAE(nn.Module):
    ...
    def encoder_(self, x, lengths):
        ...
        x = pack_padded_sequence(x, lengths.view(-1).cpu(), batch_first=True,enforce_sorted=False)
        # print("Shape after pack_padded_sequence: ", x.data.size())

        x_ = self.enc_notes_gru(x)[-1].data
        # print("Shape after enc_notes_gru: ", x_.size())
        x = self.enc_notes_gru(x)[-1].transpose(0, 1).contiguous()
        # print("Shape after enc_notes_gru: ", x.size())
        ...
        # print("Shape after enc_time_gru: ", x.size())
    ...
    def decoder_(self, z, inference, x, lengths, teacher_forcing_ratio1,
    ...
    def forward(self, x, inference=False, sample=True,
        ...
        # print("Input shape after index_tensor_to_multihot_tensor: ", x.size())
        dist, embedded_x = self.encoder_(x, lengths)
        if sample:
            z = dist.rsample()
        else:
            z = dist.mean
        if inference:
            pitch_outs, dur_outs = self.decoder_(z, inference, None, None,
                                                0., 0.)
        else:
            pitch_outs, dur_outs = self.decoder_(z, inference,
                                                embedded_x, lengths,
    ...
    def encoder(self, x):
        ...
        with torch.no_grad():
            lengths = self.get_len_index_tensor(x)
            x = self.index_tensor_to_multihot_tensor(x)
            dist, _ = self.encoder_(x, lengths)
        return dist

    def decoder(self, z):
        self.eval()
        with torch.no_grad():
            pitch_outs, dur_outs = self.decoder_(z, True, None, None, 0., 0.)
        est_pitch = pitch_outs.max(-1)[1].unsqueeze(-1)
        est_dur = dur_outs.max(-1)[1]
        recon = torch.cat([est_pitch, est_dur], dim=-1)

        sos_line = torch.zeros(recon.size(0), self.num_step, 1, 6, dtype=recon.dtype, device=recon.device)
        sos_line[:, :, :, 0] = self.pitch_sos
        sos_line[:, :, :, 1:] = self.dur_pad
    ...
    def init_model(cls):
        ...
        return cls(device=device, dec_dur_hid_size=64)

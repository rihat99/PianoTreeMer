...
def loss_function(recon_pitch, 
    ...
    recon_dur = recon_dur.view(-1, 7, 2)
    dur = dur.view(-1, 7)
    ...
    dur5 = dur_criterion(recon_dur[:, 5, :], dur[:, 5])
    dur6 = dur_criterion(recon_dur[:, 6, :], dur[:, 6])

    # dur_loss = dur_criterion(recon_dur, dur)  # (bs * 32 * 15 * 5)
    # dur_loss = dur_loss.view(-1, 5)  # some hard code here
    w = torch.tensor([1, 0.6, 0.4, 0.3, 0.3, 0.3, 0.3],
                      dtype=float,
                      device=recon_dur.device)
    # dur_loss = (dur_loss * w).mean()
    dur_loss = w[0] * dur0 + w[1] * dur1 + w[2] * dur2 + w[3] * dur3 + w[4] * dur4 + w[5] * dur5 + w[6] * dur6
    kl_div = kl_divergence(dist, normal).mean()
    loss = weights[0] * pitch_loss + weights[1] * dur_loss + weights[2] * kl_div
...
class MinExponentialLR(ExponentialLR):
    ...
    def get_lr(self):
        return [max(base_lr * self.gamma ** self.last_epoch, self.min) for base_lr in self.base_lrs]
...
def target_to_3dtarget(pr_mat, 
    ...
    pr_mat3d = np.ones((128, max_note_count, 8), dtype=int) * dur_pad_ind
    pr_mat3d[:, :, 0] = pitch_pad_ind
    pr_mat3d[:, 0, 0] = pitch_sos_ind
    cur_idx = np.ones(128, dtype=int)
    for t, p in zip(*np.where(pr_mat != 0)):
        pr_mat3d[t, cur_idx[t], 0] = p - min_pitch
        binary = np.binary_repr(int(pr_mat[t, p]) - 1, width=7)
        pr_mat3d[t, cur_idx[t], 1: 8] = \
            np.fromstring(' '.join(list(binary)), dtype=int, sep=' ')
        cur_idx[t] += 1
    pr_mat3d[np.arange(0, 128), cur_idx, 0] = pitch_eos_ind
    return pr_mat3d
